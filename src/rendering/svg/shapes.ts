/**
 * SVG shape rendering for annotations (arrows, circles, labels)
 */
import type { State } from '../../engine/state';
import type { Key, Pos, Color, NumberPair, Elements } from '../../core/types';
import type { DrawShape, DrawShapePiece, DrawBrush, DrawBrushes, DrawModifiers, Drawable } from '../../interactions/draw-handler';
import { sameEndpoints, sameColor } from '../../interactions/draw-handler';
import { key2pos } from '../../core/squares';

/** Shape synchronization hash */
export type Hash = string;

/** A shape ready for syncing with the DOM */
...
type AngleSlots = Set<AngleSlot>;
type ArrowDests = Map<Key | undefined, AngleSlots>;

export { createElement, setAttributes };

/**
 * Creates the SVG defs element with filters
 */
...
/**
 * Renders all SVG shapes
 */
export function renderSvg(state: State, els: Elements): void {
...
function syncDefs(d: Drawable, shapes: SyncableShape[], els: Elements) {
...
function renderShape(
  ...
  if (shape.customSvg) {
    ...
    el.appendChild(parseCustomSvg(shape.customSvg.html));
...
/**
 * Parses custom SVG HTML and wraps it in an SVG element.
 * Note: This accepts trusted content from the library consumer for custom shape rendering.
 */
function parseCustomSvg(html: string): SVGElement {
  const svgWrapper = createElement('svg');
  setAttributes(svgWrapper, { width: '1', height: '1', viewBox: '0 0 100 100' });
  // Use DOMParser for safer parsing of trusted SVG content
  const parser = new DOMParser();
  const doc = parser.parseFromString(`<svg xmlns="http://www.w3.org/2000/svg">${html}</svg>`, 'image/svg+xml');
  const parsed = doc.documentElement;
  // Transfer children from parsed SVG to our wrapper
  while (parsed.firstChild) {
    svgWrapper.appendChild(parsed.firstChild);
  }
  return svgWrapper;
}
...
const orient = (pos: Pos, color: Color): Pos => (color === 'white' ? pos : [7 - pos[0], 7 - pos[1]]);
...
const isShort = (dest: Key | undefined, dests: ArrowDests): boolean =>
  !!dest && dests.has(dest) && anyTwoCloserThan90Degrees(dests.get(dest)!);

const createElement = (tagName: string): SVGElement =>
  document.createElementNS('http://www.w3.org/2000/svg', tagName);

const angleCount = (dest: Key | undefined, dests: ArrowDests): number =>
...
function pos2user(pos: Pos, bounds: DOMRectReadOnly): NumberPair {
  const xScale = Math.min(1, bounds.width / bounds.height);
  const yScale = Math.min(1, bounds.height / bounds.width);
  return [(pos[0] - 3.5) * xScale, (3.5 - pos[1]) * yScale];
}

function filterBox(from: NumberPair, to: NumberPair): SVGElement {
  const box = {
...
const moveAngle = (from: NumberPair, to: NumberPair): number =>
  Math.atan2(to[1] - from[1], to[0] - from[0]) + Math.PI;

const dist = (from: NumberPair, to: NumberPair): number =>
  Math.sqrt([from[0] - to[0], from[1] - to[1]].reduce((acc, x) => acc + x * x, 0));

function labelCoords(from: NumberPair, to: NumberPair, slots?: AngleSlots): NumberPair {
...
/**
 * Syncs shapes with the DOM (append and remove only, no updates)
 */
export function syncShapesSimple(
  shapes: SyncableShape[],
